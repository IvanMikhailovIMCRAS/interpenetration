...
if __name__ == "__main__":
    ...
    for sigma1 in [0.01, 0.03, 0.05, 0.07]:
        ...
        lower_limit = int(N1 * sigma1 + N2 * sigma2) + 2
        upper_limit = int((MP.H1 + MP.H2) * 1.2)
        for D in range(lower_limit + 7, upper_limit):
            try:
            ...
            with open(file_path, "ab") as f:
                f.write(
                    msgpack.packb(
                        data_block, default=ndarray_encoder, use_bin_type=True
                    )
                )
